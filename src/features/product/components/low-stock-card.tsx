...
export function LowStockCard({ products }: Props) {
  return (
    <div className='flex flex-col space-y-3 sm:space-y-4 rounded-2xl sm:rounded-3xl border border-amber-200/60 bg-white p-4 sm:p-6 shadow-sm transition-all hover:shadow-md'>
      {/* Card Header */}
      <div className='flex items-center justify-between border-b border-amber-100 pb-3 sm:pb-4'>
        <div className='flex items-center gap-2.5 sm:gap-3'>
          <div className='flex h-9 w-9 sm:h-11 sm:w-11 items-center justify-center rounded-xl sm:rounded-2xl bg-amber-50 text-amber-600 shadow-2xs ring-2 sm:ring-4 ring-amber-50/50 shrink-0'>
            <AlertTriangle className='h-5 w-5 sm:h-6 sm:w-6' />
          </div>
          <div>
            <div className='flex items-center gap-1.5 sm:gap-2'>
              <h3 className='text-sm sm:text-lg font-bold text-slate-900'>Low Stock Alerts</h3>
              <span className='rounded-full bg-amber-100 px-2 py-0.5 text-[10px] sm:text-xs font-bold text-amber-800'>
                {products.length}
              </span>
            </div>
        ...
        <Link
          to='/products'
          className='flex items-center gap-1 rounded-xl bg-slate-50 px-2.5 py-1 sm:px-3 sm:py-1.5 text-xs font-bold text-slate-600 transition hover:bg-amber-50 hover:text-amber-700 cursor-pointer shrink-0'
        >
          <span>All</span>
      ...
      <div className='flex-1 overflow-x-auto rounded-xl sm:rounded-2xl border border-slate-100'>
        {products.length === 0 ? (
          <div className='flex flex-col items-center justify-center p-6 text-center space-y-1.5 bg-emerald-50/40 rounded-xl'>
            ...
            <p className='text-xs sm:text-sm font-bold text-emerald-900'>Stock Levels Healthy!</p>
          ...
          <table className='w-full border-collapse text-left text-xs'>
            <thead>
              <tr className='border-b border-slate-200 bg-slate-50/80 font-bold uppercase tracking-wider text-slate-500 text-[10px] sm:text-xs'>
                ...
                <th className='px-3 py-2.5 sm:px-4 sm:py-3 text-center'>Stock / Min</th>
              ...
              {products.slice(0, 10).map((product) => (
                <tr key={product.id} className='transition hover:bg-amber-50/30'>
                  ...
                  <td className='px-3 py-2.5 sm:px-4 sm:py-3.5 text-center'>
                    <span className='inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-0.5 sm:px-2.5 sm:py-1 text-[10px] sm:text-[11px] font-extrabold text-amber-800'>
