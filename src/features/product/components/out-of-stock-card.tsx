...
export function OutOfStockCard({ products }: Props) {
  return (
    <div className='flex flex-col space-y-4 rounded-3xl border border-rose-200/60 bg-white p-6 shadow-sm transition-all hover:shadow-md'>
      {/* Card Header */}
      <div className='flex items-center justify-between border-b border-rose-100 pb-4'>
        <div className='flex items-center gap-3'>
          <div className='flex h-11 w-11 items-center justify-center rounded-2xl bg-rose-50 text-rose-600 shadow-2xs ring-4 ring-rose-50/50'>
            <AlertOctagon size={22} />
          </div>
          <div>
            <div className='flex items-center gap-2'>
              <h3 className='text-lg font-bold text-slate-900'>Out of Stock Items</h3>
        ...
        <Link
          to='/products'
          className='flex items-center gap-1 rounded-xl bg-slate-50 px-3 py-1.5 text-xs font-bold text-slate-600 transition hover:bg-rose-50 hover:text-rose-600 cursor-pointer'
        >
          <span>View All</span>
      ...
      <div className='flex-1 overflow-x-auto rounded-2xl border border-slate-100'>
        {products.length === 0 ? (
          <div className='flex flex-col items-center justify-center p-8 text-center space-y-2 bg-emerald-50/40 rounded-2xl'>
            ...
            <p className='text-sm font-bold text-emerald-900'>Zero Out of Stock Items!</p>
