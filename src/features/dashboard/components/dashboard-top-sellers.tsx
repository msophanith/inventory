        ...
        <div className='space-y-3'>
          {topSellers.map((item, i) => (
            <div key={item.productId} className='flex items-center gap-3'>
              <div
                className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${RANK_COLORS[i] || 'from-slate-400 to-slate-300'} text-white text-xs font-black shadow-sm`}
              >
                {i + 1}
              </div>
              <div className='flex-1 min-w-0'>
                <div className='flex items-center justify-between mb-1'>
                  ...
                  <span
                    className={`text-[11px] font-extrabold shrink-0 ml-2 ${RANK_TEXT[i] || 'text-slate-600'}`}
                  >
                    {item.qtySold} sold
                  </span>
                </div>
                <div className='relative h-1.5 w-full rounded-full bg-slate-100 overflow-hidden'>
                  <div
                    className={`absolute left-0 top-0 h-full rounded-full bg-gradient-to-r ${RANK_COLORS[i] || 'from-slate-400 to-slate-300'} transition-all duration-700`}
