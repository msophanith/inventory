      ...
      <div className='rounded-3xl border border-slate-200/80 bg-white p-5 sm:p-6 shadow-sm space-y-4'>
        <div className='flex items-center justify-between border-b border-slate-100 pb-3.5'>
          <div className='flex items-center gap-2.5'>
            ...
            <div>
              <h2 className='font-extrabold text-slate-900 text-base'>{t('reports.stockDistribution')}</h2>
              <p className='text-xs text-slate-500 font-medium'>{t('reports.stockDistributionDesc')}</p>
            </div>
          </div>
        ...
        <div className='space-y-2 pt-2'>
          <div className='flex h-4 w-full overflow-hidden rounded-full bg-slate-100 shadow-inner'>
            <div style={{ width: `${healthyPct}%` }} className='bg-emerald-500 transition-all duration-500' title={`Healthy: ${healthyCount} (${healthyPct}%)`} />
            <div style={{ width: `${lowPct}%` }} className='bg-amber-400 transition-all duration-500' title={`Low: ${lowStock} (${lowPct}%)`} />
            <div style={{ width: `${outPct}%` }} className='bg-rose-500 transition-all duration-500' title={`Out: ${outOfStock} (${outPct}%)`} />
          </div>

          <div className='grid grid-cols-3 gap-2 pt-3 text-center'>
            <div className='rounded-2xl border border-slate-100 bg-slate-50/70 p-3'>
              <span className='inline-block h-2 w-2 rounded-full bg-emerald-500 mb-1' />
              <p className='text-xs font-extrabold text-slate-900'>{healthyCount} {t('reports.items')}</p>
              <p className='text-[11px] font-semibold text-slate-400'>{t('reports.inStock')} ({healthyPct}%)</p>
            </div>
            <div className='rounded-2xl border border-amber-100 bg-amber-50/40 p-3'>
              ...
              <p className='text-xs font-extrabold text-amber-900'>{lowStock} {t('reports.items')}</p>
              <p className='text-[11px] font-semibold text-amber-600'>{t('reports.lowStockCount')} ({lowPct}%)</p>
            </div>
            <div className='rounded-2xl border border-rose-100 bg-rose-50/40 p-3'>
              ...
              <p className='text-xs font-extrabold text-rose-900'>{outOfStock} {t('reports.items')}</p>
              <p className='text-[11px] font-semibold text-rose-600'>{t('reports.outOfStockCount')} ({outPct}%)</p>
            </div>
          </div>
        </div>
      </div>
