      ...
      <div className='w-full max-w-sm max-h-[90vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl bg-white p-4 pb-8 sm:p-6 shadow-2xl space-y-4 animate-in slide-in-from-bottom sm:zoom-in-95 duration-200'>
        ...
        <div className='rounded-2xl border border-slate-100 bg-slate-50 p-3 text-xs space-y-2.5 max-h-52 overflow-y-auto'>
          {receipt.items.map((i) => {
            ...
            return (
              <div key={i.product.id} className='flex flex-col gap-1 border-b border-slate-200/60 pb-2 last:border-0 last:pb-0'>
                <div className='flex justify-between font-bold text-slate-800'>
                  <span>{i.quantity}x {i.product.name}</span>
                  <span className='font-extrabold text-slate-900'>{formatCurrencyUsd(i.totalPrice)}</span>
                </div>
                <div className='flex items-center justify-between text-[11px] text-slate-500 font-medium'>
                  <span className='flex items-center gap-1 text-slate-400'><PackageCheck size={12} /> Remaining Stock:</span>
                  <span className={`font-bold rounded-full px-2 py-0.5 text-[10px] ${isLow ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'}`}>
        ...
        <div className='border-t border-slate-100 pt-3 space-y-1.5 text-xs text-slate-600 font-medium'>
          ...
          <div className='flex justify-between'>
            ...
            <span className='font-bold text-slate-900'>{receipt.soldBy || 'Admin'}</span>
          </div>
          <div className='flex justify-between items-baseline text-sm font-extrabold text-slate-900 pt-1'>
            <span>Total Amount</span>
            <div className='text-right'>
              <span className='text-emerald-600 block'>{formatCurrencyUsd(receipt.total)}</span>
              <span className='text-[10px] text-indigo-600 block'>{formatCurrencyKhr(receipt.total)}</span>
            </div>
          </div>
          ...
          <div className='flex justify-between text-slate-500'>
            <span>Change</span>
            <span>{formatCurrencyUsd(receipt.change)} ({formatCurrencyKhr(receipt.change)})</span>
          </div>
        </div>
        ...
        <div className='flex flex-col gap-2 pt-2'>
          <div className='flex gap-2'>
            ...
            >
              <FileText size={16} /> {isGeneratingPdf ? 'Generating...' : 'Preview PDF'}
            </button>
            <button
              onClick={handlePrint}
