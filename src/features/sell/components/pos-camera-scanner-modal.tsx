  ...
  useEffect(() => {
    ...
    const startScanner = async () => {
      try {
        ...
        const html5QrCode = new Html5Qrcode('pos-camera-viewfinder', { formatsToSupport: BARCODE_FORMATS, verbose: false });
      ...
      <div className='w-full max-w-sm max-h-[90vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl bg-white p-4 pb-8 sm:p-5 shadow-2xl space-y-4 animate-in slide-in-from-bottom sm:zoom-in-95 duration-200'>
          ...
          <div className='flex items-center gap-2'>
            {isTorchSupported && (
              <button
                ...
                className={`flex h-8 w-8 items-center justify-center rounded-xl transition cursor-pointer ${isTorchOn ? 'bg-amber-400 text-slate-950 shadow-md ring-2 ring-amber-300' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
              >
                {isTorchOn ? <Zap size={16} className='fill-slate-950' /> : <ZapOff size={16} />}
              </button>
            )}
            <button onClick={onClose} className='rounded-xl p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition cursor-pointer'><X size={20} /></button>
          </div>
        </div>
        ...
        <form onSubmit={handleManualSubmit} className='space-y-2'>
          ...
          <div className='relative'>
            ...
            />
            <button type='submit' className='absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-emerald-600 cursor-pointer'><Search size={16} /></button>
          </div>
        </form>
      </div>
    </div>
